...
nombre_plats = 7 #int(input("Nombre de plats? "))
...
with open("sortie.csv", "w", encoding="utf-8") as sortie:
    for plat in service:
        ...
        if (plat["niveau"] < diff_min or plat["niveau"] > diff_max) and (plat["prix"] < prix_min or plat["prix"] > prix_max): #marqueur
            service.remove(plat)
    ...
    lignes = ["JOURS", "NOMS", "PRIX", "TEMPS", "DIFFICULTE", "URL"]
    for i in range(nombre_plats):
        try:
            ...
            lignes[0] += ";"+jours[i]
            lignes[1] += ";"+selection["nom"]
            lignes[2] += ";"+str(selection["prix"])+" sur 3"
            lignes[3] += ";"+selection["temps"]+"m"
            lignes[4] += ";"+str(selection["niveau"])+" sur 4"
            lignes[5] += ";"+selection["url"]
